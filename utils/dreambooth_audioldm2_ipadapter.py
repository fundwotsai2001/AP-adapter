...
def log_validation(audioldmpipeline, text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, global_step,vocoder,concept_audio_dir, validate_experiments=False):
    ...
    for _ in range(args.num_validation_audio_files):
        ...
        audio_gen = pipeline(args.validation_prompt,negative_prompt = "worst quality, low quality",num_inference_steps=50,audio_length_in_s=10.0).audios[0]
        class_gen = pipeline(args.class_prompt, negative_prompt = "worst quality, low quality",num_inference_steps=50,audio_length_in_s=10.0).audios[0]
        audios.append(audio_gen)
        class_audios.append(class_gen)
        ...
        for i, audio in enumerate(class_audios):
            # scipy.io.wavfile.write(os.path.join(val_audio_dir, f"{'_'.join(args.validation_prompt.split(' '))}_{i}.wav"), rate=16000, data=audio)
            write(os.path.join(val_audio_dir, f"{'_'.join(args.class_prompt.split(' '))}_{i}.wav"),16000, audio)
...
def wav_to_mel(
    ...
    if augment_data:
        ...
        mel = mel.squeeze(0)
    return mel
def get_audio_embeds(wav_file=None, LOA_embedding=None):
    ...
    model.eval()
    LOA_embed = model(mel_spect_tensor, time_pool=1, freq_pool=1)
    uncond_LOA_embed = model(torch.zeros_like(mel_spect_tensor), time_pool=1, freq_pool=1)
...
class AudioInversionDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        #     return_tensors="pt",
        # ).input_ids[0]
        LOA_embeds, uncond_LOA_embeds = get_audio_embeds(
            wav_file=audio_file, LOA_embedding=None
        )
        bs_embed, seq_len, _ = LOA_embeds.shape
        LOA_embeds = LOA_embeds.repeat(1, 1, 1)
        LOA_embeds = LOA_embeds.view(bs_embed , seq_len, -1)
        uncond_LOA_embeds = uncond_LOA_embeds.repeat(1, 1, 1)
        uncond_LOA_embeds = uncond_LOA_embeds.view(bs_embed , seq_len, -1)
        rand_num = random.random()
        if rand_num < 0.05:
            LOA_embeds = uncond_LOA_embeds
        elif rand_num < 0.1:
            text = ""
        elif rand_num < 0.15:
            text = ""
        ...
        # uncond_LOA_embeds, _ = generated_prompt_embeds.chunk(2)
        ...
        generated_prompt_embeds = torch.cat([generated_prompt_embeds, LOA_embeds], dim=1)
...
def main():
    ...
    for name, param in audioldmpipeline.language_model.named_parameters():
        if param.requires_grad:
            print(name)
    with open('grad_param_ipadapter.txt', 'w') as file:
        for name, param in unet.named_parameters():
            if param.requires_grad:
                file.writelines(name)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                # print("generated_prompt_embeds",generated_prompt_embeds.shape)
                # print("attention mask",type(attention_mask))
                # print("encoder_hidden_states",generated_prompt_embeds.shape)
                ...
                # model_pred = noise_pred_uncond + 3.5 * (noise_pred_text - noise_pred_uncond)
